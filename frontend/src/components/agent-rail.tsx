...
export function AgentRail({
  ...
  return (
    <Card className="border-border bg-background p-4 transition-colors duration-500 ease-in-out md:p-5">
      <div className="space-y-5">
        <div>
          <p className="font-mono text-[11px] font-medium uppercase tracking-[0.3em] text-muted">Agent Activity</p>
          <h3 className="mt-3 text-2xl leading-none tracking-tight text-foreground">Execution Rail</h3>
        </div>
        <div className="space-y-3">
          {labels.map((item) => {
            ...
            return (
              <div key={item.key} className="border-t border-border pt-4 first:border-t-0 first:pt-0">
                ...
                <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.24em] text-muted">{status}</p>
                <p className="mt-2 break-words font-mono text-xs leading-7 text-muted">{state?.detail ?? "Waiting to start."}</p>
              </div>
            );
          })}
          {extraStates.map((item) => (
            <div key={item.label} className="border-t border-border pt-4">
              ...
              <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.24em] text-muted">{item.status}</p>
